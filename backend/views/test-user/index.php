	...
	<div class="modal-dialog">
		<div class="modal-content">
          ...
          <div id="last_ip_div" class="form-group">
              ...
              <div class="col-sm-10">
<label class="checkbox-inline"><input type="checkbox" name="TestUser[last_ip][]" id="inlineCheckbox1" value="127.0.0.1">本机</label>
<label class="checkbox-inline"><input type="checkbox" name="TestUser[last_ip][]" id="inlineCheckbox1" value="192.168.1.1">内网</label>
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="is_online_div" class="form-group">
              ...
              <div class="col-sm-10">
<select class="form-control" name="TestUser[is_online]" id="is_online"><option value="y">是</option><option value="n">否</option></select>              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="domain_account_div" class="form-group">
              ...
              <div class="col-sm-10">
<input type="hidden" class="form-control" id="domain_account" name="TestUser[domain_account]" /><input id="domain_account_file" type="file" name="domain_account_file" class="file" data-overwrite-initial="true" data-min-file-count="1">              </div>
              <div class="clearfix"></div>
          </div>
...
function orderby(field, op){
 ...
 function initEditSystemModule(data, type){
	if(type == 'create'){
        ...
        $("#is_online").prop("selectedIndex", 0)
        $("#domain_account").val("");
        $("#status").val("");
        $("#create_user").val("");
        $("#create_date").val("");
...
function initModel(id, type, fun){
	...
	$.ajax({
		   ...
		   success: function(data){
			   ...
			   ////////////////////////////////////////
   	               $("#domain_account_file").fileinput('destroy').fileinput({
	                   language: 'zh', //设置语言
	               ...
	               .on('fileselect', function(event, numFiles, label) {
	                   ...
	                   // 只考虑上传一个文件
	                   if($('#domain_account_div .kv-preview-thumb').size() > 1){
	                       $('#domain_account_div .kv-preview-thumb').first().remove();
	                   }
	               })
	               .on("fileuploaded", function(event, data,previewId,index){
	                   var result = data.response;
	                   $("#domain_account").val(result.image);
	               })
